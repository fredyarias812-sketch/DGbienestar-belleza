...
const CONFIG = {
  ...
  whatsapp    : "523312345678",  // ← número real de Diana (52 + 10 dígitos, sin + ni espacios)
  duracionMin : 60,
  pin         : "0922",
...
// ╔══════════════════════════════════════════════════════╗
// ║  API — usa GET con JSONP para evitar CORS           ║
// ╚══════════════════════════════════════════════════════╝
function apiGET(params) {
  return new Promise((resolve) => {
    ...
    // Intentamos fetch normal primero
    fetch(url)
      .then(r => r.json())
      .then(data => resolve(data))
      .catch(() => {
        // Si falla por CORS, usamos script tag (JSONP-like workaround)
        console.warn("fetch falló, intentando script tag");
        resolve(null);
      });
  });
}
...
function apiPOST(payload) {
  return new Promise((resolve) => {
    ...
    fetch(CONFIG.sheetURL, {
      ...
      .then(r => r.json())
      .then(data => resolve(data))
      .catch(err => { console.error("POST error:", err); resolve(null); });
  });
}
...
async function guardarCita(cita) {
  // Intenta POST primero, si falla intenta GET con params
  let res = await apiPOST({ action: "addCita", cita });
  if (!res) {
    res = await apiGET({ action: "addCita", cita: JSON.stringify(cita) });
  }
  return res;
}

async function eliminarCita(id) {
  let res = await apiPOST({ action: "deleteCita", id });
  if (!res) res = await apiGET({ action: "deleteCita", id });
  return res;
}

async function toggleBloqueoAPI(dateKey, bloqueado) {
  let res = await apiPOST({ action: "toggleBloqueo", dateKey, bloqueado });
  if (!res) res = await apiGET({ action: "toggleBloqueo", dateKey, bloqueado: String(bloqueado) });
  return res;
}
...
function parseDateKey(k) {
  const [y,m,d] = k.split("-"); return new Date(+y,+m-1,+d);
}
...
function renderCalendar() {
  ...
  for(let d=1;d<=days;d++){
    const date=new Date(y,mo,d), key=getDateKey(date);
...
function renderSlots(date,key){
  const wrap=document.getElementById("slotsWrap"),title=document.getElementById("slotsTitle"),
  ...
  slots.forEach(({time,taken})=>{
    ...
    if(!taken){ btn.addEventListener("click",()=>{
      document.querySelectorAll(".slot-btn").forEach(b=>b.classList.remove("selected"));
      btn.classList.add("selected"); sel.value=time;
      document.getElementById("agendar").scrollIntoView({behavior:"smooth",block:"start"});
    });}
    grid.appendChild(btn);
    if(!taken){const o=document.createElement("option");o.value=time;o.textContent=time;sel.appendChild(o);}
  });
...
document.getElementById("apptForm").addEventListener("submit", async function(e){
  ...
  if(CONFIG.sheetURL){
    ...
    if(res&&res.ok){
      ...
      toast("¡Cita guardada en todos los dispositivos! ✓","success");
    } else {
      // Guardado falló pero igual abrimos WA y guardamos local
      appointments.push(cita);
      toast("Cita registrada. Verifica tu conexión.","");
    }
  } else {
    appointments.push(cita);
...
function renderDiasBloqueados(){
  const wrap=document.getElementById("diasBloqueadosWrap"); if(!wrap) return;
  ...
  wrap.innerHTML=lista.map(k=>`
    <div class="dia-bloq-item">
      ...
      <button class="btn-reactivar" onclick="handleToggleBloqueo('${k}')">Reactivar</button>
    </div>`).join("");
}

function renderMiniCal(){
  const grid=document.getElementById("miniCalGrid"),label=document.getElementById("miniCalLabel");
  if(!grid) return; grid.innerHTML="";
  const y=viewDate.getFullYear(),mo=viewDate.getMonth();
  const today=new Date(); today.setHours(0,0,0,0);
  label.textContent=`${MESES[mo]} ${y}`;
  const first=new Date(y,mo,1).getDay(),days=new Date(y,mo+1,0).getDate();
  for(let i=0;i<first;i++){const e=document.createElement("div");e.className="mini-cal-day empty";grid.appendChild(e);}
  for(let d=1;d<=days;d++){
    const date=new Date(y,mo,d),key=getDateKey(date);
    const el=document.createElement("div"); el.textContent=d; el.className="mini-cal-day";
...
function renderAppointments(){
  ...
  const hoy=getDateKey(new Date());
  const up=sorted.filter(a=>a.fecha>=hoy), past=sorted.filter(a=>a.fecha<hoy);
  let html="";
  if(up.length){html+=`<p class="appt-section-label">Próximas citas (${up.length})</p>`;html+=up.map(apptCard).join("");}
  if(past.length){html+=`<p class="appt-section-label muted">Anteriores (${past.length})</p>`;html+=past.map(a=>apptCard(a,true)).join("");}
  list.innerHTML=html;
}
...
async function borrarCita(id){
  ...
  if(CONFIG.sheetURL) await eliminarCita(id);
...
(function setMinDate(){
  ...
  document.getElementById("fecha").addEventListener("change",function(){
    const [y,m,d]=this.value.split("-"); selectDay(new Date(+y,+m-1,+d),this.value);
  });
})();
...
setInterval(()=>{ if(CONFIG.sheetURL) cargarDatos(); }, 90000);
